...
def draw_rect_value(surface: pygame.Surface, x: int, y: int,
                    w: int, h: int, value: float, label: str | None = None) -> None:
...
def draw_temp_box(surface: pygame.Surface, x: int, y: int,
                  w: int, h: int, temp: float, label: str) -> None:
    pygame.draw.rect(surface, T()["panel_bg"], (x, y, w, h), border_radius=20)
    pygame.draw.rect(surface, T()["border"],   (x, y, w, h), width=3, border_radius=20)
...
def draw_segment_bar(surface: pygame.Surface, x: int, y: int,
                     w: int, h: int, pct: float,
                     mode: str = "heat", segments: int = 40,
                     gap: int = 1, border_radius: int = 0) -> None:
    pygame.draw.rect(surface, T()["fill_bg"], (x, y, w, h), border_radius=border_radius)
    pygame.draw.rect(surface, T()["border"],  (x, y, w, h), width=3, border_radius=border_radius)
    p         = max(0, min(100, pct)) / 100.0
    ...
    for i in range(segments):
        ...
        rel  = (i + 0.5) / segments
        col  = (heat_color_inverted(rel * 100.0) if mode == "heat"
                else cool_color(rel * 100.0))
        if i < lit_full:
            pygame.draw.rect(surface, col,           rect, border_radius=border_radius // 2)
        else:
            pygame.draw.rect(surface, (210, 210, 210), rect, border_radius=border_radius // 2)
        pygame.draw.rect(surface, T()["border"], rect, width=1,
                         border_radius=border_radius // 2)


def draw_banner(surface: pygame.Surface, text: str, color: tuple,
                cx: int, cy: int) -> None:
...
def draw_button(surface: pygame.Surface, rect: pygame.Rect,
                label: str, font: pygame.font.Font | None = None) -> None:
